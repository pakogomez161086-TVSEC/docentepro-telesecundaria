...
CREATE POLICY "Docentes gestionan sus examenes" ON public.examenes FOR ALL TO authenticated
USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Administradores ven todos los examenes" ON public.examenes FOR SELECT TO authenticated
USING (public.has_role(auth.uid(), 'administrador'));
...
CREATE TRIGGER trg_examenes_updated BEFORE UPDATE ON public.examenes FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
